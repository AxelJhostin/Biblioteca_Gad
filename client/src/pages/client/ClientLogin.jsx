...
export default function ClientLogin() {
  ...
  if (user) return <Navigate to={user.debe_cambiar_password ? '/mi-cuenta/seguridad' : location.state?.from?.pathname || '/mi-cuenta'} replace />;

  const submit = async (event) => {
    event.preventDefault(); setError(''); setLoading(true);
    try {
      const nextUser = await login(form.identificacion, form.password);
      const destination = nextUser.debe_cambiar_password ? '/mi-cuenta/seguridad' : location.state?.from?.pathname || '/mi-cuenta';
