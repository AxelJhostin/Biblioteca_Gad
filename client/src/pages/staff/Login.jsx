...
export default function Login() {
  ...
  return <div className="bg-auth"><Link to="/" className="auth-back-button"><i className="fas fa-arrow-left" /><span>Volver al catálogo</span></Link><div className="auth-shell">
    ...
    <div className="auth-form"><div className="text-center mb-4"><div className="auth-logo" data-easter-trigger><img src="/assets/logo.jpg" alt="Logo municipal" /></div><h4 className="mt-3 mb-1">Acceso del personal</h4><p className="text-muted">Biblioteca Municipal de Jipijapa</p></div>
      {error && <div className="alert alert-danger">{error}</div>}
      <form onSubmit={submit}><label className="form-label" htmlFor="staff-user">Usuario</label><div className="input-group mb-3"><span className="input-group-text"><i className="fas fa-user" /></span><input id="staff-user" className="form-control" required autoFocus autoComplete="username" value={form.usuario} onChange={(e) => setForm({ ...form, usuario: e.target.value })} /></div><label className="form-label" htmlFor="staff-password">Contraseña</label><div className="input-group mb-4"><span className="input-group-text"><i className="fas fa-lock" /></span><input id="staff-password" className="form-control" required type={show ? 'text' : 'password'} autoComplete="current-password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /><button className="btn btn-outline-success" type="button" onClick={() => setShow(!show)} aria-label={show ? 'Ocultar contraseña' : 'Mostrar contraseña'}><i className="fas fa-eye" /></button></div><button className="btn btn-primary w-100 py-2" disabled={loading}>{loading ? 'Ingresando…' : 'Ingresar'}</button></form>
