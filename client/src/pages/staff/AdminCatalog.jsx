...
import { validateAuthors } from '../../lib/catalogValidation.js';
...
export default function AdminCatalog() {
  ...
  const edit = async (book) => {
    ...
    setSelected(book.id); setShowForm(true); setCover(null); setDigital(null); setAuthorsError('');
    setForm({
      ...empty,
      ...data.item,
  ...
  const create = () => { setSelected(null); setForm(empty); setCover(null); setDigital(null); setAuthorsError(''); setShowForm(true); };
  ...
  const submit = async (event) => {
    ...
    const authorValidation = validateAuthors(form.autores_texto);
    if (authorValidation.error) {
      setAuthorsError(authorValidation.error);
      return;
    }
    setAuthorsError('');
    const payload = { ...form, autores: authorValidation.authors, cantidad_total: Number(form.cantidad_total), anio_publicacion: form.anio_publicacion || '' };
  ...
  return <><PageHeader icon="fa-book" title="Catálogo" subtitle={user?.rol === 'administrador' ? 'Gestiona obras, existencias y archivos digitales' : 'Consulta portadas, identificadores y disponibilidad'} actions={<div className="d-flex flex-wrap align-items-center gap-2"><ReportActions type="inventario" filters={search ? { search } : {}} />{user?.rol === 'administrador' && <button className="btn btn-primary" onClick={create}><i className="fas fa-plus me-2" />Nuevo libro</button>}</div>} />
    {showForm && <form className="card mb-4" onSubmit={submit}><div className="card-header d-flex justify-content-between"><span>{selected ? 'Editar libro' : 'Registrar libro'}</span><button className="btn-close" type="button" onClick={() => setShowForm(false)} /></div><div className="card-body"><div className="row g-3">
      ...
      <div className="col-md-6"><label className="form-label" htmlFor="book-authors">Autores separados por coma</label><input id="book-authors" className={`form-control${authorsError ? ' is-invalid' : ''}`} required value={form.autores_texto} aria-describedby="book-authors-help book-authors-error" onChange={(e) => { setForm({ ...form, autores_texto: e.target.value }); setAuthorsError(''); }} />{authorsError ? <div id="book-authors-error" className="invalid-feedback">{authorsError}</div> : <div id="book-authors-help" className="form-text">Escribe cada nombre con al menos 2 caracteres.</div>}</div><div className="col-md-3"><label className="form-label">Tipo</label><select className="form-select" value={form.tipo_material} onChange={(e) => setForm({ ...form, tipo_material: e.target.value })}><option value="libro">Libro</option><option value="revista">Revista</option><option value="folleto">Folleto</option><option value="tesis">Tesis</option><option value="otro">Otro</option></select></div><div className="col-md-3"><label className="form-label">Género</label><select className="form-select" value={form.genero} onChange={(e) => setForm({ ...form, genero: e.target.value })}><option value="narrativa">Narrativa</option><option value="lirico">Lírico</option><option value="poesia">Poesía</option><option value="ensayo">Ensayo</option><option value="otro">Otro</option></select></div>
