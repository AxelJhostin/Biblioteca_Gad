...
const cards = [
  ...
  ['listos_retiro', 'Listos para retirar', 'fa-bell', 'linear-gradient(135deg,#4EA8DE,#277DA1)'],
...
export default function Dashboard() {
  ...
  return <><PageHeader icon="fa-gauge-high" title="Inicio" subtitle="Resumen operativo de la Biblioteca Municipal" />
    <div className="row g-3 mb-4">{cards.map(([key, label, icon, gradient]) => <div className="col-sm-6 col-xl" key={key}><div className="stat-card" style={{ background: gradient }}><div><small>{label}</small><strong>{data.metrics[key]}</strong></div><i className={`fas ${icon}`} /></div></div>)}</div>
    <div className="card"><div className="card-header d-flex justify-content-between"><span>Requieren atención</span><Link to="/panel/prestamos">Ver préstamos</Link></div><div className="table-responsive"><table className="table align-middle mb-0"><thead><tr><th>Código</th><th>Cliente</th><th>Materiales</th><th>Fecha</th><th>Estado</th></tr></thead><tbody>{data.attention.map((item) => <tr key={item.id}><td className="fw-bold">{item.codigo}</td><td>{item.nombre_completo}</td><td>{item.titulos.join(', ')}</td><td>{formatDateTime(item.fecha_solicitud)}</td><td><span className={`badge bg-${stateClass(item.estado)}`}>{stateLabel(item.estado)}</span></td></tr>)}{!data.attention.length && <tr><td colSpan="5" className="text-center text-muted py-4">No hay operaciones pendientes.</td></tr>}</tbody></table></div></div>
  </>;
}
