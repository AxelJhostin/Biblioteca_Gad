...
export default function Loans() {
  ...
  return <><PageHeader icon="fa-right-left" title="Préstamos" subtitle="Registra entregas directas y devoluciones parciales o completas" actions={<div className="d-flex flex-wrap align-items-center gap-2"><select aria-label="Filtrar préstamos por estado" className="form-select loan-filter" value={filter} onChange={(e) => setFilter(e.target.value)}><option value="">Todos</option><option value="activo">Activos</option><option value="atrasado">Atrasados</option><option value="devuelto">Devueltos</option><option value="rechazado">Rechazados</option></select><ReportActions type="prestamos" filters={filter ? { estado: filter } : {}} /><button className="btn btn-primary" onClick={() => setShowDirect(true)}><i className="fas fa-plus me-2" />Nuevo préstamo</button></div>} />
